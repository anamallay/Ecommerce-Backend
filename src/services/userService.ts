...
export const getUsers = async (pageParam: number, limitParam: number, searchParam: string) => {
  ...
  const payload: IUser[] = await User.find(searchFilter, { password: 0 }).skip(skip).limit(limit)
